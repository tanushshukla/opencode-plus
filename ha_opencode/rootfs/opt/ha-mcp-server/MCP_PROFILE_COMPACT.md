...
The built-in `homeassistant` MCP server is in the **compact** profile. It exposes only read-only state, history, diagnostics, templates, calendars, and focused home-context tools. Use `get_home_context` before broad state listings.
